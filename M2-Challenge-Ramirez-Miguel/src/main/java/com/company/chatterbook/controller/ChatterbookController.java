...
public class ChatterbookController {
    ...
    //Return a given user in JSON format
    @RequestMapping(value = "/users/{user}", method = RequestMethod.GET)
    @ResponseStatus(value = HttpStatus.OK)
    public User getUser(@PathVariable String user) {
        User foundUser = null;
        for (User checkUser : userList) {
            if (checkUser.getName().toUpperCase().trim().equals(user.toUpperCase().trim())) {
    ...
    //Return a given user's posts in JSON format
    @RequestMapping(value = "/posts/{user}", method = RequestMethod.GET)
    @ResponseStatus(value = HttpStatus.OK)
    public List<ChatterPost> getUserPosts(@PathVariable String user) {
        User foundUser = null;
        for (User checkUser : userList) {
            if (checkUser.getName().toUpperCase().trim().equals(user.toUpperCase().trim())) {
